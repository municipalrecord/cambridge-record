...
# REC (reconsiderations) joined 2026-08-09: the series was always in
# the docket, but the grammar never knew it, so "REC-2025-1" in an
# anchor was no claim at all — and a readings-queue row could not
# name its owner in a way this audit could hear.
STEM = re.compile(r"\b(POR|PO|CMA|AR|ARS|APP|RES|ORD|COF|CHS|HS|HR|COM|REC)"
                  r"[  -](\d{4})[  ]?[-#](\d+)\b")
